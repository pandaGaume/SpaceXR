...
export class MorganKeenanClass {
    private static pattern = /^(O|B|A|F|G|K|M)([0-9](.[0-9])?)((Ia\+|I|II|III|IV|V|sd|D))?$/;
    public static LuminosityNames: MKLuminosity[] = ["Ia+", "I", "II", "III", "IV", "V", "sd", "D"];

    public static Parse(str: string): MorganKeenanClass | undefined {
        const a = this.pattern.exec(str);
        if (a) {
            const major = <MKMajor>a[0];
            const minor = parseFloat(a[1]);
            const lum = <MKLuminosity>(a.length > 2 ? a[2] : undefined);
    ...
    public constructor(major: MKMajor, minor: MKMinor, luminosity?: MKLuminosity) {
        this._major = major;
        this._minor = minor;
    ...
    public get luminosity(): MKLuminosity | undefined {
        return this._lum;
    }
...
export class SpectralClass {
    public static O = new SpectralClass(
        "O",
        new QR(new Temperature(30000, Temperature.Units.k), new Temperature(60000, Temperature.Units.k)),
        "blue",
        "blue",
    ...
    public static B = new SpectralClass(
        "B",
        new QR(new Temperature(10000, Temperature.Units.k), new Temperature(30000, Temperature.Units.k)),
        "blue white",
        "deep blue white",
    ...
    public static A = new SpectralClass(
        "A",
        new QR(new Temperature(7500, Temperature.Units.k), new Temperature(10000, Temperature.Units.k)),
        "white",
        "blue white",
    ...
    public static F = new SpectralClass(
        "F",
        new QR(new Temperature(6000, Temperature.Units.k), new Temperature(7500, Temperature.Units.k)),
        "yellow white",
        "white",
    ...
    public static G = new SpectralClass(
        "G",
        new QR(new Temperature(5200, Temperature.Units.k), new Temperature(6000, Temperature.Units.k)),
        "yellow",
        "yello white",
    ...
    public static K = new SpectralClass(
        "K",
        new QR(new Temperature(3700, Temperature.Units.k), new Temperature(5200, Temperature.Units.k)),
        "orange	pale",
        "yello orange",
    ...
    public static M = new SpectralClass(
        "M",
        new QR(new Temperature(2400, Temperature.Units.k), new Temperature(3700, Temperature.Units.k)),
        "red light",
        "orange red",
    ...
    public static HarwardClassification: SpectralClass[] = [SpectralClass.O, SpectralClass.B, SpectralClass.A, SpectralClass.F, SpectralClass.G, SpectralClass.K, SpectralClass.M];

    static TemperatureRange: QR<Temperature> = new QR(new Temperature(2400, Temperature.Units.k), new Temperature(60000, Temperature.Units.k));

    static ClassFromTemperature(temperature: Temperature | number): SpectralClass | undefined {
        const temp: Temperature = new Temperature(temperature, Temperature.Units.k);
        const c: SpectralClass[] = SpectralClass.HarwardClassification;

        for (let i = 0; i !== c.length; i++) {
            const sc: SpectralClass = c[i];
            const min: Temperature = sc.effectiveTemperature.min;
            const max: Temperature | undefined = sc.effectiveTemperature.max;
