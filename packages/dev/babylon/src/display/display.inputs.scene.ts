import { EventState, IDisposable, Nullable, Observer, PointerEventTypes, PointerInfo, Scene, Vector2, Vector3 } from "@babylonjs/core";
import { VirtualDisplay } from "./display.virtual";
import { Cartesian2WithInfos } from "core/map/inputs";

export class VirtualDisplayInputsControllers implements IDisposable {
    ...
    public constructor(display: VirtualDisplay) {
        ...
        this._scenePointerObserver = this._display.getScene().onPointerObservable.add(this.onPointerEvent.bind(this));
    ...
    public dispose(): void {
        if (this._scenePointerObserver) {
            this._display.getScene().onPointerObservable.remove(this._scenePointerObserver);
