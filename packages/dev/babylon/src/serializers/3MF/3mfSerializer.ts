...
export interface IBjsThreeMfSerializerOptions extends IThreeMfSerializerOptions {
    ...
    /**
     * If true, export submeshes as distinct objects/resources when supported.
     * If false, submeshes may be merged into a single object depending on implementation.
     */
...
export class BjsThreeMfSerializer extends AbstractThreeMfSerializer<Mesh | InstancedMesh, IBjsThreeMfSerializerOptions> {
    ...
    static DefaultOptions: IBjsThreeMfSerializerOptions = { unit : ST_Unit.meter, exportInstances:false, exportSubmeshes:false };
    ...
    public override toModel(builder:ThreeMfModelBuilder, meshes: Array<Mesh | InstancedMesh>): I3mfModel {
        ...
        for (let j = 0; j < meshes.length; j++) {
            ...
            if (this.options.exportSubmeshes) {
                ...
                // Note: if there are no submeshes, nothing is exported here.
                // Depending on your expectations, you may want to fall back to exporting the whole mesh.
                // Kept as-is to preserve current behavior.
            } else {
                // Export the whole mesh as a single 3MF object.
                const data = {
                ...
                for (let j = 0; j < _instances.length; j++) {
                    ...
                    if (this.options.exportSubmeshes && subMeshes && subMeshes.length > 0) {
                        for (let k = 0; k < subMeshes.length; k++) {
                            ...
                            if (objectRef) {
                                modelBuilder.withBuild(
                                    objectRef.id,
                                    this._handleBjsTo3mfMatrixTransformToRef(worldTransform, Matrix3d.Zero())
                                );
                                continue;
                            }

                            // If objectRef is missing, we skip silently.
                            // This could happen if _extractSubMesh returned undefined earlier.
                        }
                        continue;
                    }

                    // If we exported whole meshes, instances reference the mesh-level object.
                    ...
                    // If objectRef is missing, the instance is skipped.
                }
            }
        }
