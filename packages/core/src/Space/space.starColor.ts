import { ParametricValue } from "../Math/math";
...
export class ColorValue {
    public constructor(public mk: MorganKeenanClass, public sclass: SpectralClass, public kelvin: number, public color: RGBAColor) {}
}
...
export class StarColor {
    ...
    private static _buildIndex(ColorTable: ColorInfoPair[]): ColorIndex {
        ...
        const o: any = {};
        for (const l of MorganKeenanClass.LuminosityNames) {
    ...
    private static *_SelectByLuminosity(l: MKLuminosity, table: ColorInfoPair[]): Iterable<ColorValue> {
        for (const p of table) {
            const c = MorganKeenanClass.Parse(p.key);
            if (c && c.luminosity === l) {
                const t = 0.9 - c.minor / 10;
                const sc = SpectralClass.HarwardClassificationIndex[c.major];
    ...
    public static lookupRgb(luminosity: MKLuminosity, temperature: Temperature | number): RGBAColor {
        const kelvin: Temperature = new Temperature(temperature, Temperature.Units.k);

        const i: ColorValue[] = this._lookupIndexes(luminosity, kelvin.value);
        ...
        const k1 = i[1].kelvin;
        const dk = k1 - k0;
        const c0: RGBAColor = i[0].color;
        if (dk == 0) return c0;
        const c1: RGBAColor = i[1].color;
        const f: ParametricValue = (kelvin.value - k0) / dk;
