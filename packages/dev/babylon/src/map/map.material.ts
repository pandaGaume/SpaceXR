...
import { ITileAddress } from "core/tiles";
import { IElevationTile } from "./map.elevation.layer";
...
class TileBag {
    ...
    public layerArea: Nullable<ITexture3Layer> = null;
...
/// </summary>
export class MapMaterial extends PushMaterial {
    public static ClassName: string = "TerrainHologramMaterial";
    public static ShaderName: string = "tilemap";

    // tile bags is holding the area information provided by TilePoolTexture for each activ tile.
    ...
    private _elevationSampler: Nullable<ITexture3>;
    private _normalSampler: Nullable<ITexture3>;
    private _layerSampler: Nullable<ITexture3>;
    ...
    constructor(name: string, options: TerrainHologramMaterialOptions, scene: Scene) {
        super(name, scene);
        // the map to listen
        this._map = map;
        // the listeners
    ...
    public isReadyForSubMesh(mesh: AbstractMesh, subMesh: SubMesh, useInstances?: boolean): boolean {
        ...
        const shaderName = MapMaterial.ShaderName;
    ...
    private _onTileAdded(eventData: IElevationTile, eventState: EventState): void {
    ...
    private _onTileUpdated(eventData: IElevationTile, eventState: EventState): void {
        const mesh = eventData.surface;
        if (mesh) {
            // setup the layer
            this._loadLayer(eventData);
    ...
    private _onTileRemoved(eventData: IElevationTile, eventState: EventState): void {
